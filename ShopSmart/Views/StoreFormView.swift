...
struct StoreFormView: View {
        ...
        NavigationStack {
            Form {
                ...
                Section("Items Carried") {
                    ...
                    ForEach(allItems, id: \.id) { (item: Item) in
                        HStack {
                            Text(item.name)
                            Spacer()
    ...
    private func save() {
        ...
        if let store {
            ...
            store.notes = finalNotes
            store.websiteURL = storeURL
            store.items = selectedItems
        } else {
            let newStore = Store(name: trimmedName, notes: finalNotes)
            newStore.websiteURL = storeURL
            modelContext.insert(newStore)
            newStore.items = selectedItems
        }
        dismiss()
    }
